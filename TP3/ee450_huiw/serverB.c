...
int main()
{
    ...
    while (1)
    {
        ...
        else // build the ID list to send back
        {
            ...
            printf("Server B has received a request for Student %s in %s\n", target_id, target_dpt);
            while (temp_dpt != NULL)
            {
            ...
            if (temp_id == NULL)
            {
                ...
                sendto(sockfd, (const char *)not_found, strlen(not_found),
                       0, (const struct sockaddr *)&clientaddr,
                       addrlen);
                printf("Server B has sent “Student %s not found” to Main Server\n", target_id);
            }
            else
            {
                ...
                strcpy(std_rec, rec);
                printf("Server B calculated following academic statistics for Student %s in %s:\n", target_id, target_dpt);
